...
public class listarProductos extends AppCompatActivity {
    ...
    ListView productos, Fabricantes, Precio;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Fabricantes = findViewById(R.id.fab);
        Precio = findViewById(R.id.prec);
            ...
            public void onResponse(JSONArray response) {
                ...
                for (int i = 0; i < response.length(); i++) {
                    try {
                        ...
                        b.add(job.getString("fabricante"));
                        c.add(job.getString("precio")+" $");
                    } catch (JSONException e) {
                        Toast.makeText(getApplicationContext(), e.toString(), Toast.LENGTH_SHORT).show();
                    }
                }
                productos.setAdapter(new ArrayAdapter<String>(getApplicationContext(), R.layout.estilo_listview, a));
                Fabricantes.setAdapter(new ArrayAdapter<String>(getApplicationContext(), R.layout.estilo_listview, b));
                Precio.setAdapter(new ArrayAdapter<String>(getApplicationContext(), R.layout.estilo_listview, c));
